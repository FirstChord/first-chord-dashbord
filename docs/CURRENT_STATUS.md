...
- **Intent-led documentation migration (2026-07-20):** repository docs now have
  one router at `docs/README.md` and are grouped by purpose: architecture,
  policies, operations, workflows, plans, reference, and history. Every document
  carries lifecycle metadata; portal-era instructions are explicitly historical;
  `npm run docs:check` and CI guard links, metadata, and the retired mixed-folder
  layout. `AGENTS.md` remains the coding-agent entry point.
...
- **Proposals inbox V1 (2026-07-19, built — deliberately parked for a later
  ...
  terms and sets
  `ADMIN_AI_REPLY_DRAFT_ENABLED`** (`docs/architecture/ai/tool-contracts.md` →
...
- **Continuity pack + the parked-prompt bug (2026-07-19, committed):** Finn
  ...
  [`OPERATIONS_CONTINUITY.md`](./operations/continuity.md) — the people-side twin
...
- **Legacy pair two (2026-07-19, deployed):** **(4) Money-path
  ...
  [`DATA_PROTECTION_MAP.md`](./policies/data-protection.md) — every PII store ×
...
- **Legacy triptych (2026-07-19, deployed):** three arcs chosen
  ...
  has an entry path), and [`DISTILLATION_PLAYBOOK.md`](./workflows/songs/distillation-playbook.md)
  ...
  [`DISASTER_RECOVERY.md`](./operations/disaster-recovery.md). **(3) Contract guards:**
...
- Earlier arcs (song requests + the `add-song` skill, song loop telemetry,
  ...
  traps live in [`docs/reference/song-catalogue-coverage.md`](./reference/song-catalogue-coverage.md) —
  the WhatsApp bridge arc, finance layer, payroll V1–2) → `git log` + Learning
  Log. Tutor-facing payroll Phase 3 remains planned in
  `docs/plans/active/tutor-payroll.md`.

## Standing context & policy

Durable rules (don't grow per session). The state-tab map is `docs/architecture/data/state-tabs.md` — treat it as canonical before adding any Sheets tab.
...
- **Overview placement:** top cards = work to do today / needs attention / deliberate school-improvement prompts; background health belongs lower down unless something's wrong; prefer human labels over big counts; don't add a panel just because data exists. (`docs/policies/copy-and-tone.md` records the language layer.)
...
- **Tutor-facing payroll — Phase 2 shipped; Phase 3 deferred:** reviewed figures produce a signed no-login statement link; tutors can confirm or query; confirmation-required rows wait, disputes are held out of Wise, and the same printable record becomes a dated receipt after payment. The live operating sequence is `docs/workflows/finance/paying-tutors.md`; design history and the remaining cadence/scheduled-delivery work are in `docs/plans/active/tutor-payroll.md`. Only Phase 3 needs persistent tutor auth/contact email.
...
- **Tutor dashboard OAuth (agreed in principle 2026-07-19, not started):** put
  ...
  [`DATA_PROTECTION_MAP.md`](./policies/data-protection.md).
- **Proposals inbox V1 — BUILT, deliberately parked for a later experiment
  ...
  accept the privacy/policy terms in `docs/architecture/ai/tool-contracts.md` before enabling.
...
Canonical/derived ownership lives in `docs/architecture/data/state-tabs.md` and
`docs/architecture/data/ownership.md`; repository-wide forbidden and approval-gated action
...
Fragile format contracts (don't change without updating the parser + tests — recorded in `docs/architecture/data/state-tabs.md` → Format Contracts):
...
3. **working on songs, grades, or paths?** `docs/reference/song-catalogue-coverage.md` — coverage, every known gap, and the ingestion traps that have already drawn blood
4. `docs/architecture/data/state-tabs.md` (state lanes); a relevant Obsidian `03 Architecture` note is optional design context
