...
This is a supporting map of repository paths, deployed services, and operator
context. It is not the agent entry point or a second current-status document.

Start with the repository-wide [AGENTS.md](../../AGENTS.md), then use
[Current status](../CURRENT_STATUS.md) and the focused documentation router.
...
- copy/tone layer: `docs/policies/copy-and-tone.md` captures the dashboard's current voice and the rule that wording should reduce cognitive load
...
2. `docs/architecture/system/admin-loop.md`
3. `docs/architecture/data/ownership.md`
4. `docs/architecture/data/state-tabs.md` — canonical state lane map
5. `docs/operations/runbook.md` — recovery, env vars, component health, backups, and deploy rollback
6. `docs/policies/workflow-design.md`
7. `docs/policies/copy-and-tone.md`
8. `docs/policies/ui-conventions.md`
9. `docs/policies/hygiene-and-secrets.md`
10. `docs/README.md`
11. `docs/policies/school.md`
12. `docs/policies/payments.md` if touching Stripe, pauses, or payment expectations
13. `docs/operations/incidents/bug-fixes.md` if debugging Railway, MMS calendar times, or recent production issues
14. `docs/workflows/practice-chat/delivery.md` if touching Practice Chat Level 2 delivery or rollout
...
For dashboard-owned Sheets tabs, use `docs/architecture/data/state-tabs.md` as the canonical state lane map. It marks each tab as truth, cache, workflow state, append-only log, or derived context.
...
- copy/tone guide: `docs/policies/copy-and-tone.md`
- UI/action guide: `docs/policies/ui-conventions.md`; new async admin actions should show pending/success/error feedback and should avoid `window.location.reload()`
...
- before widening beyond Finn/Tom/Fennella/Dean, update and act on `docs/workflows/practice-chat/delivery.md`; caller identity, caller/student authorisation, config-driven rollout, and duplicate-send concurrency are blockers
...
Before editing docs, check `docs/README.md`. It defines which repo docs are current authority and which older docs are historical background.
