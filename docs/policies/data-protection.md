...
7. **Reply drafting sends redacted parent message text to OpenAI** — the
   ...
   `docs/architecture/ai/tool-contracts.md` → "Incoming reply drafting — sign-off notes".

---

*Maintenance: new store or new purpose → new row here, same session (same discipline as
`docs/architecture/data/state-tabs.md`). The retention report reads its policy from this table's
