...
| `GITHUB_DASHBOARD_REPO` | Overrides the dashboard's own GitHub repo name for registry writes and workflow health | Optional; defaults to `FirstChord/first-chord-dashbord`, the name GitHub serves today | Set this to the new `owner/name` at the moment the repository is renamed, so registry writes keep landing without waiting for a deploy; unset it to roll back. GitHub redirects a GET after a rename, but the registry write is a PUT where a redirect can drop the body, so the exact name is always required. Also update the token scope in `GITHUB_TOKEN` and the default in `lib/admin/github-repos.mjs`. |
