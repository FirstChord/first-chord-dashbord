...
| Finance snapshots and payroll context | Read-only finance estimate, snapshots, Stripe collection calibration, expenses, payroll review, and Wise preparation. Owner: Finn/Tom. | `/admin` shows healthy Finance Snapshot, Stripe Amounts, and Finance Data cards; the current month has one monthly `Finance_Snapshot` baseline; the previous month has a `Stripe_Collected_Monthly` row; `Payroll_Runs` reflects review/payment state; every estimate/calibration basis is labelled. | Re-run either workflow with its normal secret after checking whether its row already exists. Monthly snapshots retry automatically on days 1–7 and are idempotent by month; correct config tabs (`Tutor_Pay`, `Expenses`) before trusting numbers. | Never backdate or fabricate a missing baseline, delete an older snapshot to force a retry, treat estimates as Stripe/accounting truth, or mark payroll paid unless Tom has actually paid it. | `/admin`; `/admin/finance`; `/admin/payroll`; `.github/workflows/finance-snapshot.yml`; `.github/workflows/stripe-amounts.yml`; `docs/plans/active/tutor-payroll.md`; finance/payroll tests. |
