...
| Waiting-list onboarding | Turns a selected Waiting student and optional exact MMS `Free` event into canonical records plus MMS activation, billing and first lesson. Owner: Finn/Tom. | Every onboarding step is visible; a Waiting-selected slot reports both first-lesson confirmation and `mmsFreeSlot` success before Waiting closes. | Before writes, return to Waiting and choose the current suggestion if Free-event validation says stale. After lesson success plus Free-event failure, inspect MMS and remove only the remaining selected `Free` event manually. | Never rerun full onboarding or recreate the lesson after `mmsFirstLesson` succeeded. Never delete a Free event before lesson confirmation, reconstruct an event ID from tutor/time, or mass-delete a recurring series through an undocumented API parameter. | `/admin/waiting`; `/admin/onboard`; `docs/architecture/data/ownership.md`; `tests/admin/mms-free-slot.test.mjs`; `tests/admin/onboarding-helpers.test.mjs`. |
