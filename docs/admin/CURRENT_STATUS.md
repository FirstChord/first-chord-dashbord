...
4. **Waiting-list capacity matching refinement** — DONE (2026-06-21) for multi-instrument coverage, ranking, synonyms, and clearer "why". **Deferred sub-slice: day/time preference matching.** Blocked on capturing availability at sign-up: the MMS note parser has no day/time field today. Sequence: (1) add a day/time availability field to the onboarding/sign-up form, (2) parse it, (3) filter `buildWaitingCapacityMatches` slots by it. Keep matches as suggestions until a placement workflow is intentionally designed.
