...
- **Bridge reconnect no longer replays the starred backlog (2026-07-07, bridge-only, uncommitted):** the `ed01962` deploy's reconnect loop made the local bridge re-post every starred message; pre-6-July-id ones reinserted as new inbox rows and a stale heartbeat showed "empty confirmed-group list" (sheet held 170 throughout). Fixed `bridge.js` to gate the starred path to live `notify` deliveries (reconnect batches cache but never re-post); restarted the bridge (heartbeat cleared the card); deleted 15 replay rows. Takes effect on bridge restart, not a Railway push. Why: [[2026-07-07 - Bridge Reconnect Replays the Starred Backlog]].
