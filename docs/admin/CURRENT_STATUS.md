...
- **Incoming inbox: bridge heartbeat + auto-archive audit (2026-07-06 evening):** from the five-point post-rollout review — two tightened, three closed without code. New single-row `Bridge_Status` tab (heartbeat every ~30min); `assessBridgeHealth` distinguishes **down** (heartbeat >2h), **alive-but-capturing-nothing** (empty group list — the day-one 401 failure shape), and **quiet** (no capture 3d+); healthy = one muted line on the inbox page, problems = amber card on `/admin`. "Auto-archived (N)" inbox filter (structural: auto-captured + ignored + no `reviewed_by`) for weekly noise-policy audits. Closed without code: sibling groups (already modelled), reply-stamping-vs-closure (evidence flag by design; rejected a seven-state enum — workflow state lives in planning), multi-intent (deferred pending evidence; full text already travels in plan notes). Why: [[2026-07-06 - Bridge Heartbeat and Auto-Archive Audit Filter]].
- Incoming inbox auto-capture from confirmed groups replaces starring (2026-07-06); incoming classifier eval harness + one-tap convert + overview presence (2026-07-06); tutor pay statement Phase 1 + Wise-CSV/window fixes + incoming-inbox loop close/group map (2026-07-01→02), monolith splits (planning + Sheets), finance layer, payroll V1+Wise pay-out, temporal reconciliation L1, roster movement → `git log` + Learning Log. Tutor-facing payroll Phases 2–3 planned in `TUTOR_FACING_PAYROLL_ROADMAP.md`.
