...
- **Hygiene pass: monolith split resumed + one SWR cache (2026-07-08, seven commits, no behaviour change):** shared `Field`/`Input`/`Select` + `SlideOverPanel` in `components/admin/ui/` (Issues/StudentDetail/Planning consume them); planning review-board filter → tested `filterPlanningItems`; and **`lib/admin/swr-cache.mjs` replaces the two hand-rolled read caches** (Sheets reads + MMS payroll attendance) with one unit-tested policy — TTL, stale-while-revalidate with contained background refresh, hard max age, coalescing, tab-scoped invalidation with a pre-write version guard. 571 → 580 tests. Repo changelogs (`docs/LEARNING_LOG.md`, `ADMIN_IMPLEMENTATION_LOG.md`) frozen per one-home-per-fact. Remaining agreed: Issues-client handler extraction (churn falling, it waits). Map: `MONOLITH_SPLIT_MAP.md`. Why: [[2026-07-08 - Hygiene Pass Resumes the Monolith Split and Unifies the SWR Cache]].
