...
- **Incoming inbox — loop close + audit + group sync (2026-07-01):** (1) `Convert to plan + draft reply` creates an idempotent linked `Planning_Items` action (`planning_<incoming_id>`), stamps `created_planning_id`, and returns a per-category WhatsApp reply draft (copy-paste only). (2) Every review action stamps `reviewed_by`/`reviewed_at`. (3) Bulk WhatsApp group sync: `npm start -- --sync-groups` imports all groups (metadata only), keeps FC ones (instrument in title, active <6mo), auto-matches by participant phone/name, and the inbox group panel confirms/ignores each. Parent-message tone standardised to "Hi/Hello/Hey" (never "Heya"). Doc: `docs/admin/WHATSAPP_INCOMING_BRIDGE.md`.
