...
- **Tutor pay statement — Phase 2: confirm/dispute (2026-07-05):** the public statement link now has **Confirm / "Something's off"** controls (no login — the signed token is the identity). Response writes to `Payroll_Runs` (`tutor_response`/`tutor_responded_at`/`tutor_note`) and shows on the payroll board (per-card banner + a confirmations tally). A **disputed** row is **held out of the Wise batch** until re-reviewed. Never moves money; admin still approves the batch. Phase 3 (cadence self-select + emailed/scheduled delivery) still needs tutor auth + `Tutor_Pay.contact_email` — see `TUTOR_FACING_PAYROLL_ROADMAP.md`. Why: [[2026-07-05 - Tutor statement Phase 2 confirm]].
- **Planning: assignment + plan modes (2026-07-05):** plans can be assigned to **Fennella** (`PLANNING_OWNERS`) with a single "Fennella" planning filter; new **`plan_mode`** field (`task` = same-day/ticked done vs `ongoing` = worked across sessions via progress log + `target_date` next meeting day), a Mode selector in the editor, and an "Ongoing" badge on cards. Mostly activated existing fields (owner/progress-log/target_date already existed). Fennella uses the shared admin login for now; scoped tutor auth deferred (see `TUTOR_FACING_PAYROLL_ROADMAP.md`). Why: [[2026-07-05 - Planning assignment + plan modes]].
- **Finance: actuals calibration + forward outlook (2026-07-04):** the revenue side now has its first actuals loop. (1) **Stripe actuals cache** — Monday cron (`/api/cron/stripe-amounts`, before the 06:00 snapshot) full-replaces `Stripe_Amounts_Cache` (per-student subscription amounts, 14-day staleness guard) and upserts `Stripe_Collected_Monthly`; the existing `stripeAmounts` seam now feeds real billing into the page + snapshot (`source` flips to `mixed`). Key verified: restricted `rk_live` reads customers/subscriptions/invoices/prices; charges/payouts denied. (2) **Estimate vs reality panel** — last month's Stripe collections vs the frozen monthly snapshot's Stripe-mode estimate, delta %. (3) **Forward outlook** — pause walk-forward + waiting-list pipeline ("potential £/mo if converted" — waiting rows have no start dates, so undated by design; seasonal seam deferred until the series matures). (4) **Coverage → issues** — `FINANCE DATA GAP` cards (source `finance_coverage`, `noRevenuePrice`/`noTutor` only) ride the system-cleared path. (5) **Assumptions version** — every snapshot's `notes` carries `PRICE_ASSUMPTIONS_VERSION` so price/rate/VAT changes can't masquerade as growth. (6) **`GET /api/admin/finance/overview`** — the whole picture as JSON for questioning/agents. Payroll Phases 2–3 unchanged (see roadmap doc).
- **Button language pass (2026-07-04):** jargon labels renamed to plain-English intent across admin surfaces — "Expect payments active/paused" (was "Set … expected"), "Clear fixed issues" (was "Resolve system-cleared"), "Remove from portal"/"Remove portal access" (was "Delete registry …"), "Archive student" (was "Archive row"), "Mark inactive in Sheets/MMS", "Mark parents messaged", "Add note", "More options"; ellipsis style unified to `…`. Rules recorded in `COPY_AND_TONE.md` → **Buttons** (label predicts outcome without internal state names; one completion verb per family; disabled buttons say why; pending `…` → success `✓`); `UI_CONVENTIONS.md` points there. `getPrimaryPaymentQuickAction` now matches on payload, not label text, so copy can change freely. hygiene-check gains an advisory "internal jargon in new UI copy" warning (it caught its own first offender during this pass).
- **Loop-closure fixes (2026-07-04):** (1) **Failed Practice Chat parent emails now become issues** — `Practice_Notes_Log` rows with `manual_follow_up_needed = TRUE` generate `practice_delivery` Issue Queue cards; "Mark follow-up handled" clears the flag (single-cell write), logs `Event_Log`, and resolves; cleared flags also ride the system-cleared path. (2) **Quiet age chips** ("12d") on issue/waiting/parent-understanding cards from existing `updated_at` — nothing under 2 days, no schema change (tutor-absence skipped: single-date workflow, not a lingering list). (3) **Month-end money seed** — third recurring planning item ("Month end: reconcile spending against the bank", incl. Expense_Log backfill nudge) via the Friday/Monday seed mechanism. Deferred by decision: `next_check_date` timers, incoming-inbox overview card, waiting-list onboard scan. Why: [[2026-07-04 - Loop Closure Fixes (Practice Delivery, Age Chips, Month-End Seed)]].
