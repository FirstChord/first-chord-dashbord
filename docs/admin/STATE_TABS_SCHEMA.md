...
- **WhatsApp auto-capture gate + staff numbers.** Auto-ingested group messages (`source = whatsapp_group_auto`) are only written when the chat is `confirmed` in `WhatsApp_Group_Map` — confirming/ignoring a group in the inbox UI is therefore also the on/off switch for its auto-capture. School-side messages are detected via the bridge's `from_me` flag plus the dashboard env `INCOMING_STAFF_PHONES` (comma-separated, any UK format; Tom's personal number lives here) and stamp `school_replied_at`/`school_replied_by` on open rows instead of creating new ones. No-signal parent messages (`general`, no dates) are auto-archived as `ignored`. Retention of auto-archived noise rows is an open decision — they accumulate in the sheet until pruned.
