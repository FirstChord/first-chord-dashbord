...
| `Communication_Log` | Passive record of parent messages copied to send from the dashboard | `message_id` | append-only (dedup by student+body within a short window) | `appendCommunicationLogRow`, `logCommunication`, `POST /api/admin/communications` | Record-only logbook, written as a fire-and-forget side-effect of existing "Copy message" buttons. No approval, no sending. Does not change any workflow. |
