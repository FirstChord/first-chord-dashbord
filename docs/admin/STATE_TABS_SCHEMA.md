...
| `Practice_Notes_Log` | Practice Chat lesson-note memory and Level 2 delivery audit/idempotency state | `note_id` for snapshots; `delivery_key` for Level 2 delivery records | snapshots append with duplicate guard; Level 2 delivery rows upsert by `delivery_key` | `appendPracticeNoteLogRow`, `upsertPracticeNoteLogRow`, `getPracticeNoteLogRows`, `POST /api/practice-notes`, `POST /api/practice-notes/mms-test` | Dashboard-owned learning/context memory. New Level 2 rows can include selected MMS attendance ID/event ID, target-selection reason, recipient, Gmail message/thread ID, send status, sent timestamp, email error, manual follow-up state, `operation_status`, and `completed_at`. `delivery_key` is derived from student + MMS attendance + note hash, so duplicate clicks/retries do not resend the same parent email. Older rows may only contain note snapshots. |
