...
and what should stay derived."** The per-tab mechanical map is `STATE_TABS_SCHEMA.md`;
...
| Google Sheets — 24 dashboard state tabs | All workflow state, logs, caches, finance config (`STATE_TABS_SCHEMA.md`) |
...
shipped in early July. `CURRENT_STATUS.md` lists "a new database to replace Sheets" under
do-not-do-next, and `STATE_TABS_SCHEMA.md` says don't harden until Sheets is a *measured*
bottleneck. The audit confirms that: nothing is measurably broken today.

## Staged migration plan

1. **Now (no DB):** this boundary is policy (below + `STATE_TABS_SCHEMA.md`); write the inbox
