...
> Use `docs/reference/song-catalogue-coverage.md` and `docs/CURRENT_STATUS.md` for current coverage
...
- Next.js 15 on Railway. **No database.** Two data planes:
  ...
  - **Google Sheets state plane:** dashboard-owned tabs with explicit lanes
    ...
    `docs/architecture/data/state-tabs.md`. Keyed-upsert helpers in `lib/admin/sheets*`. Standing rule:
...
- `SOUNDSLICE_AGENT_HANDOVER.md` (external handover; not currently stored in
  this repository) — already sketches the right target model
...
- Touched: `app/dashboard/page-client.js`, `components/student-portal/StudentDashboard.js`
  (slice 3), `docs/architecture/data/state-tabs.md` (slice 2), `GLOSSARY.md`.
...
- Repo docs after every meaningful slice: update `docs/CURRENT_STATUS.md` (add +
...
- **Sheets tab `Song_Assignments`** (workflow-state lane, keyed upsert):
  ...
  `buildRow` → read mapping → register in `docs/architecture/data/state-tabs.md` (lane, key, write
