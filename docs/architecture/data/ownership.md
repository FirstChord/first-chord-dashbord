...
|---|---|---|---|---|---|---|---|
| Add student | transitional split ownership | `Students` sheet + MMS + registry | onboarding workflow + external APIs | `/admin/onboard` | Creates Sheets row, registry entry, MMS activation, billing profile, first lesson | Registry credentials and strict registry readability are checked before any write. A later partial failure is reported by lane and never closes Waiting or queues post-onboarding follow-ups. If Sheets exists without registry, block a full rerun and use the `SHEETS ONLY` registry repair before separately verifying MMS. | Current V1 flow has explicit preflight, partial-state reporting, and narrow registry recovery; cross-provider writes are still not transactional |
...
| First Chord lesson series/event/participation ID | authoritative identity, derived facts | First Chord PostgreSQL identity; MMS for observed schedule/attendance facts | read-only mirror command | No browser editor in phase 1 | Future reconciliation and consumer joins | A provider-reference change creates a separate FC entity until reviewed evidence supports an alias/merge; do not heuristically merge reschedules | No current operational reader uses these IDs |
| Mirrored lesson schedule/attendance fields | derived provider observation | MMS | verified whole-school read, transactional SQL upsert | Operator command only in phase 1 | Parity reporting later | Preserve provider ref, sync window and raw unknown values; an incomplete run writes no snapshot and disappearance never means cancellation | Rebuildable mirror, not an MMS replacement yet |
...
- The SQL lesson mirror is a separate event-grain parity foundation. It does not
  feed this read model or change its current ownership in phase 1.
