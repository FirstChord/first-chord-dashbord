/** @fileoverview The only bulk Stripe walk: paginated read-only subscription and invoice pulls for the scheduled amounts-cache refresh. */
...
async function listAll(path, params = {}) {
  ...
  for (let page = 0; page < MAX_PAGES; page += 1) {
    ...
    const response = await fetchWithProviderTimeout(`${STRIPE_API_BASE}${path}?${search.toString()}`, {
      headers,
      cache: 'no-store',
    }, {
      provider: 'Stripe',
