...
export async function getIssueQueueRows() {
  ...
  const rows = mapRowsToObjectsWithRowNumbers(response.data.values || []).map((row) => ({
...
export async function upsertPlanningItemRow(row) {
  ...
  const values = await getSheetValues(PLANNING_ITEMS_SHEET);
  const [, ...rows] = values;
  const planningIdIndex = headers.findIndex((header) => header === 'planning_id');
  const targetRowIndex = rows.findIndex((entry) => `${entry[planningIdIndex] || ''}`.trim() === row.planningId);
  ...
  if (targetRowIndex === -1) {
    await sheets.spreadsheets.values.append({
      spreadsheetId,
      range: buildSheetRange(PLANNING_ITEMS_SHEET, 'A:A'),
      valueInputOption: 'USER_ENTERED',
      insertDataOption: 'INSERT_ROWS',
      requestBody: {
        values: [nextRow],
    ...
    invalidateSheetReadCache(PLANNING_ITEMS_SHEET);
    return;
  }
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId,
    range: buildSheetRange(PLANNING_ITEMS_SHEET, `A${rowNumber}:${endColumn}${rowNumber}`),
    valueInputOption: 'USER_ENTERED',
    requestBody: {
      values: [nextRow],
    },
