/** @fileoverview Builds the transcription prompt biasing speech recognition toward the student's instrument and their own shelf songs. */
...
const MAX_SONGS = 12;
const MAX_UNLISTED_SONGS = 6;
const MAX_CATALOGUE_SONGS = 400;
...
/**
 * Public, non-student-specific catalogue metadata for the tutor's deliberate
 * search fallback. The transcription prompt still receives only the current
 * shelf; this wider list is for human review after the note exists.
 */
export function selectCatalogueSongs({
  catalogue = SONGS_CATALOGUE,
  maxSongs = MAX_CATALOGUE_SONGS,
} = {}) {
  return Object.entries(catalogue)
    .map(([songId, song]) => ({
      ...
      contentType: clean(song?.contentType || 'song'),
    }))
    .filter((song) => song.songId && song.title)
    .sort((a, b) => a.title.localeCompare(b.title) || a.artist.localeCompare(b.artist) || a.songId.localeCompare(b.songId))
    .slice(0, maxSongs);
}
...
export function resolveSelectedPracticeNoteSongs({
  ...
  const catalogueById = new Map(
    Object.entries(catalogue)
      .map(([songId, song]) => [songId, { songId, title: clean(song?.title) }])
      .filter(([, song]) => song.songId && song.title),
  );
  const unknown = requested.filter((songId) => !catalogueById.has(songId));
  if (unknown.length) {
    return {
      songIds: [],
      songTitles: [],
      errors: [`Selected songs are not in the First Chord catalogue: ${unknown.join(', ')}`],
    };
  }

  return {
    songIds: requested,
    songTitles: requested.map((songId) => catalogueById.get(songId).title),
    errors: [],
  };
}

/**
 * Titles outside the catalogue are observations, never canonical Song facts.
 * Preserve the tutor's wording for later curation while bounding the payload.
 */
export function resolveUnlistedPracticeNoteSongs({ titles = [] } = {}) {
  const raw = Array.isArray(titles) ? titles : `${titles || ''}`.split(',');
  const cleaned = [...new Set(raw.map((title) => clean(title).slice(0, 120)).filter(Boolean))];
  if (cleaned.length > MAX_UNLISTED_SONGS) {
    return {
      titles: [],
      errors: [`Add no more than ${MAX_UNLISTED_SONGS} unlisted songs to one Practice Note`],
    };
  }
  return { titles: cleaned, errors: [] };
}
...
export function buildPracticeChatMusicContext({
  ...
  return {
    instrument,
    songs,
    catalogueSongs: selectCatalogueSongs({ catalogue }),
    songTitles,
