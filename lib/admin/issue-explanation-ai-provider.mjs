/** @fileoverview Server-only provider call generating an AI issue briefing behind the contract's validation. */
...
export async function generateIssueAiBriefing(explanation, {
    ...
    response = await fetchImpl(OPENAI_RESPONSES_URL, {
      ...
      body: JSON.stringify({
        ...
        instructions: [
          ...
          'Character limits: headline 90, explanation 360, whatToCheck 240, caveat 220. Use plain, compact sentences.',
        ].join(' '),
  ...
  if (!validation.valid) {
    throw new IssueAiBriefingError(
      'invalid_briefing',
      'The AI briefing failed local validation',
      validation.errors,
    );
  }
