/** @fileoverview Postgres-backed delivery claims giving each practice note a single owner, with stale-claim expiry. */
import { Pool } from 'pg';

const CLAIM_TABLE = 'practice_note_delivery_claims';
export const PRACTICE_NOTE_DELIVERY_CLAIM_STALE_MS = 15 * 60 * 1000;
let sharedPool = null;
...
export async function claimPracticeNoteDelivery({
  deliveryKey = '',
  actorTutor = '',
  query = null,
  env = process.env,
  now = new Date(),
  staleAfterMs = PRACTICE_NOTE_DELIVERY_CLAIM_STALE_MS,
} = {}) {
  ...
  if (!Number.isFinite(nowTime) || !Number.isFinite(staleAfterMs) || staleAfterMs <= 0) {
    throw new Error('A valid claim time and stale threshold are required');
  }
  const execute = query || ((sql, params) => getPool(env).query(sql, params));
  const insert = await execute(`
    ...
    RETURNING delivery_key, actor_tutor, status, created_at, updated_at
  `, [key, tutor]);
  if (insert.rows?.[0]) return { ok: true, claimed: true, ...insert.rows[0] };

  // A crashed request may have reached MMS or Gmail before it could finalise.
  // Atomically park an old claim for manual reconciliation; never reacquire it
  // and risk replaying provider work whose result is unknown.
  const staleBefore = new Date(nowTime - staleAfterMs).toISOString();
  const abandoned = await execute(`
    UPDATE ${CLAIM_TABLE}
    SET status = 'tracking_failed', updated_at = NOW()
  ...
  const existing = await execute(
    `SELECT delivery_key, actor_tutor, status, created_at, updated_at FROM ${CLAIM_TABLE} WHERE delivery_key = $1`,
    [key],
  );
