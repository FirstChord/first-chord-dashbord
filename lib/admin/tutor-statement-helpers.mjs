...
export function renderTutorStatementText(statement = {}) {
  ...
  const parts = [
    `First Chord Music School — ${statement.documentType === 'receipt' ? 'payment receipt' : 'payment statement'}`,
...
export function renderTutorStatementHtml(statement = {}) {
  ...
  <h2 style="margin:4px 0 2px;font-size:20px">${statement.documentType === 'receipt' ? 'Payment receipt' : 'Payment statement'}</h2>
