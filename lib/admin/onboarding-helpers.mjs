...
export function buildOnboardingCompletionStatus({ steps }) {
  ...
  const registryReady = steps.registryWrite?.status === 'succeeded' || steps.registryWrite?.status === 'skipped';
  const mmsActivated = steps.mmsActivation?.status === 'succeeded';
  const billingReady = steps.mmsBillingProfile?.status === 'succeeded';
  const firstLessonReady = steps.mmsFirstLesson?.status === 'succeeded';
