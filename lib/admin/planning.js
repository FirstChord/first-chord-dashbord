...
export async function savePlanningItem({ planningId = '', item = {}, actorEmail = '', progressNote = '' }) {
  ...
  const proposedType = normalisePlanningItemType(item.itemType ?? existing.itemType);
  const proposedParentId = `${item.parentPlanningId ?? existing.parentPlanningId ?? ''}`.trim();
  const parent = existingRows.find((row) => row.planningId === proposedParentId);
  if (proposedType === 'action' && parent && normalisePlanningItemType(parent.itemType) === 'initiative') {
    const currentAction = findOpenProjectAction(existingRows, proposedParentId, planningId);
    if (currentAction) {
      throw new Error(`Project already has a current action: ${currentAction.title}`);
    }
  }
