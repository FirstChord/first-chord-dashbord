/** @fileoverview Builds read-only school revenue, cost, margin, and break-even summaries. */
...
// Changing this? Bump PRICE_ASSUMPTIONS_VERSION in finance-assumptions.mjs.
...
export function resolveStudentRevenue(student = {}, { stripeAmounts = null } = {}) {
  ...
  if (actual && Number.isFinite(actual.weekly)) {
    return {
      ...
      confidence: 'high',
      // Stripe knows the amount but not the lesson shape — keep the estimate's
      // classification so the by-lesson-kind breakdown stays meaningful.
      lessonKind: actual.lessonKind || derivePaymentValueContext(student).lessonKind,
      priced: true,
    };
  }
...
export function buildFinanceSnapshotRow(overview, { periodType = 'weekly', at = new Date(), roster = {} } = {}) {
  const { revenue, totals } = overview;
  const monthKey = at.toISOString().slice(0, 7);
  return {
    snapshot_id: periodType === 'monthly' ? `fs_monthly_${monthKey}` : `fs_${at.getTime()}_${periodType}`,
    ...
    onboarded_count: Number.isFinite(roster.onboarded) ? roster.onboarded : '',
    left_count: Number.isFinite(roster.left) ? roster.left : '',
    ...
    // Assumption basis, so a step-change in the series can be traced to a price/rate
    // change rather than misread as growth or churn.
    notes: PRICE_ASSUMPTIONS_VERSION,
  };
}
