...
export function createSwrCache({
  ...
  async function read(key, fetcher, { force = false, allowExpired = false } = {}) {
    if (force) {
      store.delete(key);
    ...
    const cached = peek(key, { keepExpired: allowExpired });
    if (cached?.isFresh) {
      return cached.value;
    }
  ...
  return { read, peek, stat, set, delete: remove, invalidateScope, clear };
}
