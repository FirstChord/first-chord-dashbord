...
// Short-lived cache for the payroll attendance search. The fetch range is fixed
// by the pay date, so re-rendering the payroll page — every time a tutor's
// window is adjusted, in particular — was re-fetching the same ~1000-row MMS
// query for no reason (the slow/fast variance Finn saw). A 60s TTL keyed by the
// query params makes repeat renders and window tweaks reuse one fetch.
const PAYROLL_ATTENDANCE_CACHE_TTL_MS = 60 * 1000;
...
export async function searchAttendanceForPayroll({
  ...
  const rows = responseBody?.ItemSubset || [];
  payrollAttendanceCache.set(cacheKey, { rows, expiresAt: Date.now() + PAYROLL_ATTENDANCE_CACHE_TTL_MS });
  return rows;
}
