...
// Paginated: /search/attendance caps each response at `limit`, and a payroll
// window was already ~950 rows against the old single-shot 1000 — one growth
// spurt from silently truncating pay. fetchAllPages walks offsets until a
// short page, and throws (rather than truncates) past its safety cap.
async function fetchPayrollAttendance({ startDate, endDate, teacherIds, limit }) {
  return fetchAllPages(async ({ offset, limit: pageLimit }) => {
    const response = await fetch(`${MMS_BASE_URL}/search/attendance?offset=${offset}&limit=${pageLimit}&fields=Charge,Payment&orderby=EventStartDate`, {
      method: 'POST',
      headers: getMmsHeaders(),
      body: JSON.stringify({
        StartDate: startDate,
...
export async function updatePayrollAttendanceStatus({
  ...
  const response = await fetch(`${MMS_BASE_URL}/events/${eventId}/attendance/${attendanceId}`, {
    method: 'PUT',
    headers: getMmsHeaders(),
    body: JSON.stringify({
