...
import { isFreeCalendarEvent, normaliseFreeCalendarSlot } from './capacity-helpers.mjs';
...
import {
  ...
  TEST_PRACTICE_NOTES_STUDENT_ID,
...
function assertTestPracticeNoteStudent(studentId) {
  if (studentId !== TEST_PRACTICE_NOTES_STUDENT_ID) {
    throw new Error('MMS Practice Notes test path is restricted to Test Studenty.');
  }
}

export async function previewPracticeNoteMmsTestWrite({ studentId, noteText, now = new Date() } = {}) {
  assertTestPracticeNoteStudent(studentId);
  ...
  const targetAttendance = selectPracticeNoteAttendanceTarget(attendanceRows, now);
  if (!targetAttendance) {
    throw new Error('No matching MMS attendance record found for Test Studenty.');
  }
  ...
  return {
    ...
    emailPayload: buildPracticeNoteEmailPayload(recipients),
  };
}

export async function executePracticeNoteMmsTestWrite({ studentId, noteText, now = new Date() } = {}) {
  const preview = await previewPracticeNoteMmsTestWrite({ studentId, noteText, now });
  const { targetAttendance, attendancePayload, emailPayload } = preview;
  ...
  const emailResponse = await fetch(`${MMS_BASE_URL}/events/${targetAttendance.eventId}/attendance/${targetAttendance.attendanceId}/emailnotes`, {
    method: 'POST',
    headers: getMmsHeaders(),
    body: JSON.stringify(emailPayload),
    cache: 'no-store',
  });
  const emailResponseBody = await parseMmsResponse(emailResponse);
  if (!emailResponse.ok) {
    const details = formatMmsErrorBody(emailResponseBody);
    throw new Error(`MMS email notes failed: ${emailResponse.status}${details ? ` - ${details}` : ''}`);
  }
