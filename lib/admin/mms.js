...
//
// This mirrors the read cache in `lib/admin/sheets/core.mjs`: a TTL, then a
// stale-while-revalidate window where a cached result is served immediately and
// refreshed in the background, plus in-flight coalescing so concurrent renders
// share one fetch. Net effect: a save never blocks on MMS.
//
...
const PAYROLL_ATTENDANCE_HARD_MAX_AGE_MS = PAYROLL_ATTENDANCE_CACHE_TTL_MS + PAYROLL_ATTENDANCE_STALE_WHILE_REVALIDATE_MS;
const payrollAttendanceCache = new Map();
const payrollAttendanceInflight = new Map();
...
// Refresh one cache key, coalescing concurrent callers onto a single MMS fetch.
function refreshPayrollAttendance({ cacheKey, startDate, endDate, teacherIds, limit }) {
  const inFlight = payrollAttendanceInflight.get(cacheKey);
  if (inFlight) return inFlight;

  const request = fetchPayrollAttendance({ startDate, endDate, teacherIds, limit })
    .then((rows) => {
      payrollAttendanceCache.set(cacheKey, { rows, cachedAt: Date.now() });
      return rows;
    })
    .finally(() => {
      payrollAttendanceInflight.delete(cacheKey);
    });

  payrollAttendanceInflight.set(cacheKey, request);
  return request;
}
...
export async function searchAttendanceForPayroll({
  ...
  const cacheKey = JSON.stringify({ startDate, endDate, limit, teacherIds: [...cleanTeacherIds].sort() });
  const args = { cacheKey, startDate, endDate, teacherIds: cleanTeacherIds, limit };

  if (forceRefresh) {
    payrollAttendanceCache.delete(cacheKey);
    return refreshPayrollAttendance(args);
  }

  const cached = payrollAttendanceCache.get(cacheKey);
  const age = cached ? Date.now() - cached.cachedAt : Infinity;

  if (cached && age <= PAYROLL_ATTENDANCE_CACHE_TTL_MS) {
    return cached.rows;
  }

  // Stale but serveable: hand back what we have and refresh behind the request, so
  // a "Mark reviewed" save never waits on a ~950-row MMS fetch. A failed background
  // refresh must not reject into the render — the stale rows still stand.
  if (cached && age <= PAYROLL_ATTENDANCE_HARD_MAX_AGE_MS) {
    refreshPayrollAttendance(args).catch((error) => {
      console.warn('MMS payroll attendance background refresh failed:', error?.message || error);
    });
    return cached.rows;
  }

  if (cached) payrollAttendanceCache.delete(cacheKey);
  return refreshPayrollAttendance(args);
}

export function clearPayrollAttendanceCacheForTests() {
  payrollAttendanceCache.clear();
  payrollAttendanceInflight.clear();
}
