...
function parseWaitingInstruments(note = '') {
  ...
  if (!rawInstrument) return { rawInstrument: '', instruments: [] };
...
async function searchCalendarEventsInRange({ studentId, startDate, endDate, limit = 100 }) {
  const studentIds = Array.isArray(studentId) ? studentId.filter(Boolean) : studentId ? [studentId] : [];
...
export async function getMmsFreeCalendarSlots({ lookaheadDays = 30, limit = 500 } = {}) {
  const start = new Date();
  const end = new Date(start.getTime() + lookaheadDays * 24 * 60 * 60 * 1000);
