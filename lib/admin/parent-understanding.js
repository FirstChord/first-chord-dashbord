/** @fileoverview Sheets-backed read and save of parent-understanding records, enriched with schedule context and logged to the event log. */
...
function hydrateState(row = {}, student = {}) {
  ...
  const understandingScore = calculateUnderstandingScore(details);
  const riskSignals = deriveParentUnderstandingRiskSignals(details);
  ...
  return {
    ...
    // Recalculate derived fields when reading. Older saved rows treated an
    // unanswered checklist item as a risk; this keeps the live queue truthful
    // without rewriting historical call notes.
    understandingScore: understandingScore.total,
    understandingLabel: understandingScore.labelText,
    riskSignals,
