/** @fileoverview Pure helpers matching a practice note to its MMS attendance record and building the attendance and email payloads. */
import { ADMIN_TUTORS } from './tutors-data.js';
import { noteMarkupToHtml } from '../notes-markup.mjs';

function clean(value = '') {
  return `${value || ''}`.trim();
}

function tutorComparable(value = '') {
  return clean(value)
...
export function normalisePracticeNotesTutorName(value = '') {
  const comparable = tutorComparable(value);
  if (!comparable) return '';

  for (const [shortName, tutor] of Object.entries(ADMIN_TUTORS)) {
    if (comparable === tutorComparable(shortName) || comparable === tutorComparable(tutor.fullName)) {
      return shortName;
    }
  }
...
export function formatPracticeNoteHtml(noteText = '') {
  return noteMarkupToHtml(noteText, { escape: escapeHtml, join: '' });
}

export function normalisePracticeNoteAttendanceStatus(value = '') {
  return value === 'AbsentNoMakeup' ? 'AbsentNoMakeup' : 'Present';
}

export function buildPracticeNoteAttendancePayload({ attendance = {}, noteHtml = '', attendanceStatus = 'Present' } = {}) {
  return {
    TeacherNote: attendance.teacherNote || '',
    ParentNote: attendance.parentNote || '',
    StudentNote: noteHtml,
    AttendanceStatus: normalisePracticeNoteAttendanceStatus(attendanceStatus),
  };
}
