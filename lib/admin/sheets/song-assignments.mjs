/** @fileoverview Sheets adapter for the song lane: assignments, the append-only status log, requests, and outcomes. */
import {
  buildSheetRange,
  ensureManagedSheet,
  getSheetValues,
...
export async function getSongAssignmentRows(mmsId = '') {
  ...
  return mapRowsToObjectsWithRowNumbers(values).map((row) => ({
    ...
    // Rows written before this column existed all came from the shelf.
    assignedVia: row.assigned_via || 'shelf',
...
export function buildSongAssignmentSheetRow(row) {
  return {
    ...
    assigned_via: row.assignedVia || 'shelf',
  };
}
...
// Read side of the outcome lane, added for song teaching history. Rows carry
// a tutor's free text about a real lesson, so callers must aggregate before
// showing them: nothing here is student-safe as written.
export async function getSongOutcomeRows() {
  const { spreadsheetId } = getSheetsEnv();
  const sheets = await getSheetsClient();
  if (!sheets || !spreadsheetId) {
    return [];
  ...
  const values = await getSheetValues(SONG_OUTCOMES_SHEET);
  return mapRowsToObjectsWithRowNumbers(values).map((row) => ({
    rowNumber: row.__rowNumber,
