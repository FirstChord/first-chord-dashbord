...
import { getFinanceSnapshotRows, getReviewFlagsRows, getStripeCollectedMonthlyRows } from './sheets';
...
// makes six uncached external round-trips (MMS + five GitHub Actions APIs). Cache
...
async function computeAdminHealthSummary() {
  const [flagRowsResult, snapshotRowsResult, collectedRowsResult] = await Promise.allSettled([
    getReviewFlagsRows(),
  ...
  const financeAutomation = snapshotRowsResult.status === 'fulfilled' && collectedRowsResult.status === 'fulfilled'
    ? buildFinanceAutomationHealth({ snapshotRows: snapshotRowsResult.value, collectedRows: collectedRowsResult.value })
    : {
        status: 'Unknown',
        detail: snapshotRowsResult.reason?.message || collectedRowsResult.reason?.message || 'Finance automation data is unavailable.',
        updatedAt: null,
      };

  const [mmsResult, configWorkflowResult, fcWorkflowResult, financeWorkflowResult, stripeWorkflowResult, scheduleWorkflowResult] = await Promise.allSettled([
    checkMmsHealth(),
