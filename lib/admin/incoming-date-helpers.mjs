/** @fileoverview Deterministic, reference-dated extraction of pause dates and durations from parent WhatsApp messages, conservative about inferring a return date. */
...
// "on the 12th" is common in an active chat where the message timestamp
// supplies the month. Resolve to the nearest current/future occurrence, with
// the same 14-day grace used for explicit month dates so recent past-tense
// references do not jump forward a month.
function inferMonthDay(day, reference) {
  for (let offset = 0; offset <= 12; offset += 1) {
    const monthStart = new Date(Date.UTC(reference.getUTCFullYear(), reference.getUTCMonth() + offset, 1));
    const candidate = toUtcDay(monthStart.getUTCFullYear(), monthStart.getUTCMonth(), day);
    if (candidate && candidate.getTime() >= reference.getTime() - 14 * DAY_MS) return candidate;
  }
  return null;
}
...
function collectDateTokens(text, reference) {
  ...
  // Bare ordinals are accepted only behind an explicit date-role word. This
  // reads "on the 12th ... back on the 19th" while refusing arbitrary numbers.
  // A following month is excluded because the explicit patterns above own it.
  const bareOrdinal = new RegExp(
    `\\b(?:on|from|until|till|til|by)\\s+(?:the\\s+)?(\\d{1,2})(?:st|nd|rd|th)\\b(?!\\s+(?:of\\s+)?(?:${MONTH_PATTERN})\\b)`,
    'giu',
  );
  for (const match of text.matchAll(bareOrdinal)) {
    push(inferMonthDay(Number(match[1]), reference), match);
  }
