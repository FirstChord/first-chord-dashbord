...
export function buildProjectActionCompletionProgress(action = {}, parent = {}, status = '') {
  if (
    status !== 'done'
    || action.itemType !== 'action'
    || !action.parentPlanningId
    || parent.itemType !== 'initiative'
    || parent.planningId === SCHOOL_FORWARD_PLANNING_ID
    || action.parentPlanningId !== parent.planningId
  ) {
    return null;
  }
  ...
  return {
    mode: 'progress',
    planningId: parent.planningId,
    progressNote: `Completed linked action: ${action.title}`,
    progressType: 'action_completed',
    nextAction: parent.nextAction === action.title ? '' : parent.nextAction,
  };
}
...
export function buildSearchText(item) {
  return [
    ...
    item.currentProjectAction?.title,
  ].join(' ').toLowerCase();
}
...
export function buildQuickCaptureItem(rawNote = '', overrides = {}, studentOptions = []) {
  ...
  return Object.prototype.hasOwnProperty.call(safeOverrides, 'itemType')
