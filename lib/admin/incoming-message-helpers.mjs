...
// bridge boundary doc). Tone mirrors buildTutorAbsenceMessage. When extracted
// dates are supplied the absence replies confirm them back to the parent.
export function buildIncomingReplyTemplate({
  ...
  switch (cat) {
    case 'one_off_absence':
      return `Hi ${parentFirst}! Thanks so much for letting us know ${studentFirst} can’t make it${friendlyStart ? ` on ${friendlyStart}` : ''} — no problem at all, these things happen.\n\nWe’ll pop it on our records, and ${tutorFirst} will pick things up nicely at the next lesson. See you then!`;
    case 'extended_absence':
    case 'absence_pause':
      if (friendlyStart && friendlyReturn) {
        return `Hi ${parentFirst}! Thanks for letting us know ${studentFirst} will be away from ${friendlyStart} — that’s absolutely fine.\n\nWe’ll get the break sorted on our side so the payment side lines up properly, and ${tutorFirst} will be all set to pick back up from ${friendlyReturn}. Have a lovely time!`;
      }
      return `Hi ${parentFirst}! Thanks for letting us know ${studentFirst} will be away${friendlyStart ? ` from ${friendlyStart}` : ' for a little while'} — that’s absolutely fine.\n\nWe’ll get the break sorted on our side so the payment side lines up properly, and ${tutorFirst} will be all set to pick back up the moment you’re back. Whenever you have the return date, just pop it over and we’ll take care of the rest.`;
    case 'summer_break':
      return `Hi ${parentFirst}! Thanks for the heads up about the summer break.\n\nWe’ll note ${studentFirst}’s last lesson before the holidays${friendlyStart ? ` (${friendlyStart})` : ''} and make sure everything’s lined up and ready for when schools go back${friendlyReturn ? ` — we’ve got ${friendlyReturn} down for getting started again` : ''}. Have a lovely summer!`;
    case 'leaving':
