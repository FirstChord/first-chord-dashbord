/** @fileoverview Pure builders for workflow-run, finance-automation, flag-freshness, and issue-evidence health summaries. */
...
// The one date/time formatter for admin surfaces. There were three, and they
// disagreed: the same timestamp rendered as "Not yet" on Planning and "—" on
// Student detail, with and without the year. The differences that were
// deliberate are now options; the rest were drift.
//
//   empty    — what a blank value shows ("Not yet" on Planning's cards).
//   withYear — off for compact cards where the year is noise.
export function formatDateTime(value, { empty = '—', withYear = true } = {}) {
  if (!value) return empty;
  const parsed = value instanceof Date ? value : parseIsoDate(value);
  // Echo an unparseable string rather than hiding it behind a dash: on an admin
  // screen the raw value is the clue to what went wrong.
  if (!parsed) return typeof value === 'string' ? value : empty;
