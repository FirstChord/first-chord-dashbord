...
export function buildFinanceAutomationHealth({ snapshotRows = [], collectedRows = [], now = new Date() } = {}) {
  const current = now instanceof Date ? now : new Date(now);
  ...
  const updatedAt = [monthlySnapshot?.snapshot_at, collected?.refreshed_at]
    .filter(Boolean)
    .sort()
    .at(-1) || null;
  ...
  if (!missing.length) {
    return {
      status: 'Healthy',
      detail: `${currentMonth} baseline and ${previousMonth} Stripe collections are present.`,
      updatedAt,
      currentMonth,
      previousMonth,
