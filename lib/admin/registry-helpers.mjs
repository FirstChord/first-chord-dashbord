/** @fileoverview Pure parsing, escaping, and block editing for the students-registry source file. */
// The inverse of escapeRegistryValue. Reading must undo what writing did, or a value
// containing a quote (O'Neil) gains a backslash on every save.
export function unescapeRegistryValue(value) {
...
export function extractValue(line) {
  ...
  return unescapeRegistryValue(line.slice(firstQuote + 1, lastQuote));
}
...
export async function readRegistrySource({
  nodeEnv,
  githubToken,
  readGithub,
  readBundled,
  ...
  const hasGithubToken = Boolean(String(githubToken || '').trim());

  if (isProduction && hasGithubToken) {
    try {
      const remoteFile = await readGithub();
  ...
  if (isProduction && !allowBundledFallback) {
