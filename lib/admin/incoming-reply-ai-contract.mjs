/** @fileoverview Model input and output boundary for incoming replies, owning the redaction of free parent text and fail-closed output validation. */
...
// docs/architecture/ai/tool-contracts.md and accepted (or not) at Finn's sign-off.

import { validateIncomingReplyDraft } from './incoming-reply-policy.mjs';

export const INCOMING_REPLY_SCHEMA_VERSION = 2;
export const INCOMING_REPLY_PROMPT_VERSION = 'incoming-reply-v2';
...
// The exact object sent to the provider. Deterministic policy context plus the
// redacted message — never raw rows, contact details or identifiers. The
// message itself is deliberately not copied into Proposals evidence.
export function buildIncomingReplyAiInput(policyContext = {}, { redactedMessage = '' } = {}) {
  ...
  const context = {
    ...
    acknowledgementOnly: Boolean(policyContext.acknowledgementOnly),
...
// Audit the bounded model context without retaining another copy of the
// parent's message. The source-text hash links the proposal back to the inbox
// row and detects staleness; the inbox remains the message's sole state home.
export function buildStoredReplyModelContext(modelInput = {}) {
  if (modelInput?.kind !== 'incoming_reply_input') return null;
  return {
    kind: 'incoming_reply_stored_model_context',
    schemaVersion: Number(modelInput.schemaVersion) || INCOMING_REPLY_SCHEMA_VERSION,
    policyCase: `${modelInput.policyCase || ''}`.slice(0, 40),
    noticeWindow: `${modelInput.noticeWindow || ''}`.slice(0, 40),
    lessonDateIso: `${modelInput.lessonDateIso || ''}`.slice(0, 10),
    acknowledgementOnly: Boolean(modelInput.acknowledgementOnly),
    allowedFacts: (modelInput.allowedFacts || []).slice(0, 8).map((fact) => ({
      id: `${fact.id || ''}`.slice(0, 60),
      statement: `${fact.statement || ''}`.slice(0, 240),
    })),
  };
