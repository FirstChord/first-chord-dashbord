/** @fileoverview Forecasts future finance effects from structured pause windows in planning items. */
import { buildFinanceScenario } from './finance-scenario.mjs';

// Reads pause planning items (created via the structured pause helper), extracts
// each pause window, and walks forward week-by-week — removing students during
// their window, returning them after — running the existing break-even/margin
// math per week. The grounded middle between the past (trend) and the
// hypothetical (what-if). Pure, read-only.
//
...
function isPauseItem(item = {}) {
  // Respect the explicit is_pause flag so a card converted to "general" drops out
  // of the forecast even if its old notes still mention pause; unset = infer.
...
export function parsePauseWindowsFromPlanning(items = []) {
  ...
  for (const item of items) {
    if (`${item.status || ''}`.trim().toLowerCase() === 'parked') continue;
