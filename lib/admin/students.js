...
import { filterOperationalStudents, isTestStudentRecord } from './test-student-helpers.mjs';
...
function normaliseStudentRow(row) {
  ...
  return {
    ...
    lessonFrequency: pickFirst(row, ['lesson_frequency', 'Lesson frequency', 'Lesson Frequency']),
    fcStudentId,
  ...
  return sheetRows
    ...
    .map((student) => {
      ...
      const mergedStudent = {
        ...
        lessonType: student.lessonType || registry?.lessonType || '',
        billingGroupId: student.billingGroupId || registry?.billingGroupId || '',
        groupPartnerMmsId: student.groupPartnerMmsId || registry?.groupPartnerMmsId || '',
        lessonFrequency: student.lessonFrequency || registry?.lessonFrequency || '',
...
export async function getAdminStudentByMmsId(mmsId) {
  ...
  const mergedStudent = {
    ...
    lessonType: sheetStudent.lessonType || registry?.lessonType || '',
    billingGroupId: sheetStudent.billingGroupId || registry?.billingGroupId || '',
    groupPartnerMmsId: sheetStudent.groupPartnerMmsId || registry?.groupPartnerMmsId || '',
    lessonFrequency: sheetStudent.lessonFrequency || registry?.lessonFrequency || '',
  ...
  return {
    ...
    isTestStudent: isTestStudentRecord({ ...sheetStudent, registry, registryEntry: registry }),
    ...lifecycle,
    hasFlags: flags.length > 0,
    flags,
  };
