/** @fileoverview Pure, framework-free formatting and option lists for the admin student-detail surface. */
...
// One home for date/time formatting (lib/admin/health-helpers.mjs). Re-exported
// here so this surface's callers and tests keep a single import.
export { formatDateTime } from './health-helpers.mjs';

export const STUDENT_INSTRUMENT_OPTIONS = [
  'Guitar',
  'Electric Guitar',
  'Piano',
  'Bass',
...
export function noteStatusLabel(note = {}) {
  ...
  if (note.emailSendStatus === 'not_sent_absent') return 'Attendance-only (no email)';
  if (note.operationStatus === 'in_progress' || note.operationStatus === 'retrying_email') return 'Delivery in progress';
  if (note.mmsAttendanceSaved) return 'Saved to MMS — delivery untracked';
  return 'Logged — delivery not tracked';
}
...
export function noteStatusClasses(note = {}) {
  ...
  if (note.emailSendStatus === 'not_sent_absent') return 'border-slate-200 bg-slate-100 text-slate-700';
