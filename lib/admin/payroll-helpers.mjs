...
export function buildPayrollPeriod({ payDate = nextWednesday(), cadence = 'weekly' } = {}) {
  ...
  const length = ['biweekly', 'bi-weekly', 'fortnightly'].includes(normalise(cadence)) ? 14 : 7;
  return {
    payDate: cleanPayDate,
    periodStart: addDays(cleanPayDate, -length),
    periodEnd: addDays(cleanPayDate, -1),
    days: length,
    cadence: length === 14 ? 'biweekly' : 'weekly',
  };
}
...
export function buildPayrollPreview({
  ...
  const savedById = new Map(savedRuns.map(normalisePayrollRunRow).filter((row) => row.payrollId).map((row) => [row.payrollId, row]));
  ...
  for (const tutor of tutors) {
    ...
    const period = buildPayrollPeriod({ payDate, cadence: payInfo.invoiceCadence });
