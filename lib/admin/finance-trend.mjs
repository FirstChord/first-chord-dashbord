/** @fileoverview Turns append-only Finance_Snapshot rows into a deduped, sorted time series with explicit gaps and week-over-week deltas. */
...
export function buildFinanceTrend(rows = [], { period = 'weekly', limit = 12 } = {}) {
  ...
  for (const row of rows) {
    ...
    const point = {
      ...
      // Gross VAT-inclusive turnover, kept under the legacy key for chart/test
      // compatibility. Net revenue is shown separately in the finance overview.
