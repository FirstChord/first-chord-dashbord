/** @fileoverview Static definitions of the holiday workflow instances, their task groups, and shared links. */
const COMMON_LINKS = {
  handbook: {
...
export const HOLIDAY_WORKFLOW_INSTANCES = [
...
const HOLIDAY_WORKFLOW_YEARS = Object.fromEntries(
  HOLIDAY_WORKFLOW_INSTANCES.map(({ season, year }) => [season, year]),
);
...
We just wanted to clarify our Christmas closing dates. The whole school will be off from Monday 21st December through Monday 4th January, with lessons returning on Tuesday 5th January. We’ll pause all recurring payments for that period. Do let us know in your group chats if you’ll be away before or after too. Looking forward to seeing you at the end of year show! ✨
...
Hope everyone is having a lovely day 😀 Easter is approaching; lessons will continue as normal through Easter and the spring school holidays. We know you’ll be juggling many things, so we’d like to remind you that our lesson cancellation policy is a minimum of *one week’s notice for payments to be paused*. If you are planning a holiday over Easter or the spring break, let us know ASAP in the lesson WhatsApp groups so we can update your lessons and payments. This applies to all bank holidays and the summer too. Christmas is our only two-week break in the year. Thanks so much, guys 💥.
...
Finn & Tom`,
  ...
  summer: {
    ...
    templates: {
      parentAnnouncement: `Hope everyone is doing great 😀! I just wanted to flag 🚩 that we are one week away from most mainstream schools in Glasgow beginning their summer holidays. As a reminder, we require one week notice to pause any lesson payments. So! To avoid any unnecessary payments being taken, please do give us your summer holiday plans ASAP ✅ (thanks if you already have 🫡). We shall be continuing lessons throughout the summer (though tutors may be off at various points for holidays). Summer can sometimes be a little different than the rest of the year in terms of schedule due to more folk being off. Thanks so much everyone in advance for your flexibility and your patience with having to deal with any _schedule tetris_ over the next few weeks 🙏. Tom and I will be working hard to ensure everything runs smoothly. Please do fire any questions at all over to us in your WhatsApp.
...
export function getHolidayWorkflow({ season = 'christmas', year } = {}) {
  const selectedSeason = WORKFLOWS[season] ? season : 'christmas';
  const selectedYear = `${year || HOLIDAY_WORKFLOW_YEARS[selectedSeason]}`;
  const definition = WORKFLOWS[selectedSeason];

  return {
    season: selectedSeason,
    seasonLabel: definition.label,
    year: selectedYear,
    title: definition.title.replace(/\b2026\b/u, selectedYear),
    seasonOptions: HOLIDAY_WORKFLOW_INSTANCES,
