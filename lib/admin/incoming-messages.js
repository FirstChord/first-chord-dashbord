...
export async function captureIncomingMessage(payload = {}, { actorEmail = '' } = {}) {
  ...
  const [students, groupMapRows, existingRows] = await Promise.all([
  ...
  if (isAutoCapture) {
    ...
    if (isSchoolStaffMessage(payload, process.env.INCOMING_STAFF_PHONES || '')) {
      ...
      for (const row of openRows) {
        await upsertIncomingMessageInboxRow({
          ...
          schoolRepliedBy: normalised.senderName || 'school',
        });
      }
      return { replyEvidence: true, stamped: openRows.length, incomingId: normalised.incomingId };
    }
  }
