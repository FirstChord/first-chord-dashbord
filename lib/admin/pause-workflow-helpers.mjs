...
function buildLoopState({ pauseSummary, paymentExpectation, stripeSnapshot }) {
  if (!pauseSummary?.hasPauseHistory) {
    return {
  ...
  if (pauseSummary.upcomingPause) {
    return {
      ...
      nextAction: 'No payment expectation change is needed until the pause window starts.',
      closureCondition: 'The pause loop opens when the scheduled pause start date arrives.',
    };
  }
  ...
  if (pauseSummary.currentlyPaused && paymentExpectation !== 'stripe_paused_expected') {
    return {
      ...
      nextAction: 'Most often this means the pause tool has recorded the pause, but the dashboard payment expectation still needs confirming. If the pause is correct, set payment expectation to Stripe paused expected. If the pause is wrong, correct Pause History first.',
  ...
  if (!pauseSummary.currentlyPaused && paymentExpectation === 'stripe_paused_expected') {
    return {
      ...
      nextAction: 'If the pause has ended, set payment expectation to Stripe active expected. If the pause dates are wrong, correct Pause History first.',
  ...
  if (!stripeSnapshot) {
    return {
      ...
      nextAction: 'Refresh live Stripe if this pause needs billing confirmation.',
...
export function buildPauseWorkflowSummary({ pauseSummary = null, paymentExpectation = '', stripeSnapshot = null } = {}) {
  ...
  const loopState = buildLoopState({ pauseSummary, paymentExpectation, stripeSnapshot });
