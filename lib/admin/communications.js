/** @fileoverview Sheets-backed reads and deduplicated appends for the parent communication log. */
...
export async function getCommunicationLogForStudent(mmsId, { limit = 10 } = {}) {
  const target = `${mmsId || ''}`.trim();
  if (!target) return [];
  const rows = await getCommunicationLogRows();
  return groupCommunicationLog(rows)
    .filter((entry) => entry.mmsId === target)
    .slice(0, limit);
}
