/** @fileoverview Server-only provider call for incoming reply drafts: one tool-free request, no storage, short timeout, no retry. */
...
// Mirrors the issue-briefing provider (docs/architecture/ai/runtime-integration.md): one tool-free
    ...
    response = await fetchImpl(OPENAI_RESPONSES_URL, {
      ...
      body: JSON.stringify({
        ...
        instructions: [
          ...
          'You may briefly acknowledge an emotional or logistical detail stated in the parent message.',
          'You may state school policy or outcomes only from the supplied allowedFacts, in your own calm wording; cite the ids you used in usedFactIds.',
          'When acknowledgementOnly is true, acknowledge the message without promising or offering any school action, payment outcome, cancellation, pause, schedule change, message, call or other follow-through. A safe close is that you will take a proper look and come back to them.',
