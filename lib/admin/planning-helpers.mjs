...
export function findOpenProjectAction(items = [], parentPlanningId = '', excludePlanningId = '') {
  const parentId = `${parentPlanningId || ''}`.trim();
  if (!parentId || parentId === SCHOOL_FORWARD_PLANNING_ID) return null;

  return (items || [])
    .map(normalisePlanningItem)
    .find((item) => (
      item.planningId !== excludePlanningId
      && item.itemType === 'action'
      && item.parentPlanningId === parentId
