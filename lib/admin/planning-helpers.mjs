...
// How a plan is worked, orthogonal to item_type (category). `task` = done on the
// day and ticked off; `ongoing` = worked across sessions — you log what you did
// and set the next meeting day to continue.
...
export function buildTutorAbsencePlanningItem({
  ...
  const hasMultiStudentEvent = (lessons || []).some((lesson) => Number(lesson.studentCount || 1) > 1);
  ...
  const noteLines = [
    ...
    hasMultiStudentEvent ? 'Tutor absence exception: multi-student MMS event — manual household handling required.' : '',
  ];
  ...
  return {
    ...
    linkedStudentId: '',
    nextAction: hasMultiStudentEvent
      ? 'Manual exception: verify every household in this multi-student MMS event, then record that it was handled. Automatic cover/cancel is blocked.'
      : enableEarlyNotice
