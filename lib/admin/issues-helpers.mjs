...
function buildPaymentReason({
  ...
  if (type === 'SUBSCRIPTION_STATE_MISMATCH') {
    ...
    return [expectationText, 'but the live Stripe billing state does not match that expectation.', currentPause, latestPauseWindow]
      .filter(Boolean)
      .join(' ');
  }

  if (type === 'PAUSE EXPECTATION MISMATCH') {
    return [currentPause || 'Pause History indicates the student is currently paused.', latestPauseWindow, 'The recorded payment expectation does not match that paused state.']
      .filter(Boolean)
      .join(' ');
  }

  if (type === 'PAUSE EXPECTATION STALE') {
    return ['The latest pause window appears to have ended, but the student is still marked as Stripe paused expected.', latestPauseWindow]
      .filter(Boolean)
      .join(' ');
  }
...
export function classifyIssue(flagType) {
  ...
  if (type === 'PAUSE EXPECTATION MISMATCH') {
    return {
      ...
      summary: 'Pause history says this student is currently paused, but the recorded payment expectation is not paused',
      recommendedAction: 'Set payment expectation to Stripe paused expected if the pause is correct, or review Pause History if the pause should no longer be active.',
      actionLabel: 'Review pause expectation',
      messageable: false,
    };
  }
...
function buildIssueReason({ type, sheetStudent = null, registryEntry = null, detail = '' }) {
  ...
  if (type === 'PAUSE EXPECTATION MISMATCH') {
    return 'Pause History indicates this student is currently paused, but the payment expectation has not been moved into a paused state yet.';
  }
