/** @fileoverview Waiting-list workflow state, welcome group messaging, and the onboarded transition. */
import { getWaitingStudents } from './mms.js';
import { isStudentOwnContact } from './planning-client-helpers.mjs';
import { getWaitingListStateRows, upsertWaitingListStateRow } from './sheets.js';
import { normaliseWaitingStatus } from './waiting-status.mjs';

export { isActiveWaitingStatus, normaliseWaitingStatus, WAITING_STATUS_OPTIONS } from './waiting-status.mjs';

export function buildWelcomeGroupMessage(student) {
  const parentFirstName = `${student?.parentFirstName || ''}`.trim() || 'there';
  const studentFirstName = `${student?.firstName || ''}`.trim() || 'the student';
  const isOwnContact = isStudentOwnContact(student);
  const studentReference = isOwnContact ? 'you' : studentFirstName;
  const possessiveReference = isOwnContact ? 'your' : 'their';
  const objectReference = isOwnContact ? 'you' : 'them';

  return `Hey ${parentFirstName}!
Thanks much for your interest in First Chord Music School! We are excited to chat and sort ${studentReference} out with a first lesson! 😃

Let’s organise a call to talk about ${possessiveReference} musical interests and goals so we can match ${objectReference} with the perfect tutor. Could you let us know which of these times works for you for a welcome call? Alternatively, feel free to just give us a ring.
...
Just let us know which day and time slot works best, and either Finn or Tom will give you a call then!

We’ll aim to get you started with lessons right away.
