/** @fileoverview Pure payment mode and expectation vocabularies with the rules deriving each from a student record. */
...
export function derivePaymentExpectation({
  explicitExpectation = '',
  paymentMode = '',
  ...
  if (normalisedPaymentMode === 'stripe') {
    const hasStripeLinkageContext = stripeCustomerId !== null || stripeSubscriptionId !== null;
    const hasStripeCustomer = `${stripeCustomerId || ''}`.trim() !== '';
    const hasStripeSubscription = `${stripeSubscriptionId || ''}`.trim() !== '';

    if (hasStripeLinkageContext && !hasStripeCustomer && !hasStripeSubscription) {
      return 'setup_pending';
    }
