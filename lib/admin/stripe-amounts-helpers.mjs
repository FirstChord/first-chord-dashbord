...
export function summariseCollectedInvoices(invoices = [], { month = '', students = [] } = {}) {
  ...
  const studentBreakdown = [...byStudent.values()]
    .map(({ amount_pence: amountPence, ...item }) => ({ ...item, amount: round(amountPence / 100) }))
...
export function buildCalibration({ collectedRows = [], snapshotRows = [], currentStripeWeekly = null, now = new Date() } = {}) {
  ...
  const snapshotsInMonth = snapshotRows
    .filter((row) => `${row.snapshot_at || ''}`.startsWith(month))
    .sort((a, b) => `${a.snapshot_at || ''}`.localeCompare(`${b.snapshot_at || ''}`));
  const monthSnapshot = snapshotsInMonth.find((row) => `${row.period_type || ''}`.trim() === 'monthly') || null;
  const weeklyFallback = snapshotsInMonth.find((row) => `${row.period_type || ''}`.trim() === 'weekly') || null;
  const estimateSnapshot = monthSnapshot || weeklyFallback;
  const weekly = estimateSnapshot ? toNumber(estimateSnapshot.revenue_stripe_weekly) : toNumber(currentStripeWeekly);
  ...
  return {
    ...
    estimateBasis: monthSnapshot ? 'monthly_snapshot' : weeklyFallback ? 'weekly_snapshot' : 'current_estimate',
    estimateSnapshotAt: estimateSnapshot?.snapshot_at || null,
    deltaPct,
  };
}

export function describeCalibrationBasis(calibration = {}) {
  const month = `${calibration.month || ''}`.trim() || 'that month';
  if (calibration.estimateBasis === 'monthly_snapshot') {
    return `Compared with the frozen monthly snapshot for ${month}.`;
  }
  if (calibration.estimateBasis === 'weekly_snapshot') {
    const captured = `${calibration.estimateSnapshotAt || ''}`.slice(0, 10);
    return `Monthly baseline unavailable; compared with the earliest weekly snapshot${captured ? ` from ${captured}` : ''}.`;
  }
  return `No frozen snapshot for ${month}; comparison uses today’s estimate.`;
}
