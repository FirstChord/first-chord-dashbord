...
function buildPauseAwareStudentContext({
  rawSheetRows = [],
  registryEntries = [],
  pauseHistoryRows = [],
  waitingRows = [],
  ...
  const sheetStudentsWithPauseCoverage = sheetStudents
    .map((student) => ({
      ...student,
  ...
  return {
    testStudentIds,
    operationalRegistryEntries,
    registryByMmsId,
    sheetStudents,
    sheetStudentsWithPauseCoverage,
  };
}

async function loadPauseAwareStudentContext() {
  ...
  return buildPauseAwareStudentContext({
    rawSheetRows,
    registryEntries,
    pauseHistoryRows,
    waitingRows,
...
export async function getPauseExpectationReconciliationPreview() {
  const { sheetStudentsWithPauseCoverage } = await loadPauseAwareStudentContext();
  const changes = buildPauseExpectationAutoSyncPlan(sheetStudentsWithPauseCoverage).map((change) => {
    const student = sheetStudentsWithPauseCoverage.find((entry) => entry.mmsId === change.mmsId);
    return {
      ...change,
      studentName: student?.fullName || change.mmsId,
    };
  });

  return {
    checkedCount: sheetStudentsWithPauseCoverage.length,
...
export async function reconcilePauseExpectations({ actorEmail = '' } = {}) {
  ...
  return applyPauseExpectationReconciliation(sheetStudentsWithPauseCoverage, {
    actorEmail,
