...
export async function getAdminIssues() {
  const [flagRows, rawSheetRows, registryEntries, queueRows, pauseHistoryRows, waitingRows, scheduleRows, practiceNoteRows] = await Promise.all([
    getReviewFlagsRows(),
    getStudentsSheetRows(),
    getRegistryEntries(),
    getIssueQueueRows(),
  ...
  const syncResult = mergeIssuesWithQueueState({
    ...
    now,
    managedSources: ['review_flags', 'payment_static', 'practice_delivery'],
  });
