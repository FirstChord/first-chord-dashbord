...
export class IssueAiBriefingError extends Error {
  constructor(code, message = 'AI briefing failed') {
    super(message);
...
export function validateIssueAiBriefing(value = {}, {
  ...
  const combinedText = [headline, explanation, whatToCheck, caveat].join(' ');
  if (containsDirectIdentifier(combinedText)) errors.push('direct_identifier_not_allowed');
  if (COMPLETED_ACTION_PATTERN.test(combinedText)) errors.push('completed_action_claim_not_allowed');
  if (PROMISED_ACTION_PATTERN.test(combinedText)) errors.push('promised_action_not_allowed');
