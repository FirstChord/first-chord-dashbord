...
// Name lookups over the same roster. They live here rather than in tutors.js
// because that module also reads Tutor_Lifecycle from Sheets, and importing it
// for a dictionary lookup drags the whole I/O path into otherwise pure helpers.
export function getTutorFullNameByShortName(shortName, roster = ADMIN_TUTORS) {
  return roster[shortName]?.fullName || '';
}

export function getTutorShortNameByFullName(fullName, roster = ADMIN_TUTORS) {
  const needle = `${fullName || ''}`.trim().toLowerCase();
  if (!needle) return '';
  return Object.entries(roster).find(([, tutor]) => tutor.fullName.trim().toLowerCase() === needle)?.[0] || '';
}
