...
export function parseExpenses(rows = []) {
  ...
  for (const row of rows) {
    ...
    lines.push({ name, amount, period, category: `${row.category ?? row.Category ?? ''}`.trim(), monthly: Math.round(monthly * 100) / 100 });
    monthlyTotal += monthly;
  }
  return { lines, monthlyTotal: Math.round(monthlyTotal * 100) / 100 };
}

function resolveTutor(student = {}) {
...
function slotBaseCost(durationMinutes, lessonKind, hourlyRate) {
  if (!Number.isFinite(durationMinutes) || durationMinutes <= 0) return null;
  let cost = hourlyRate * (durationMinutes / 60);
  if (lessonKind === 'group' && durationMinutes === 45) cost += GROUP_SLOT_UPLIFT;
  return cost;
}
...
export function buildTutorCost(students = [], { tutorPay = new Map() } = {}) {
  ...
  for (const slot of slots.values()) {
    ...
    const baseCost = slotBaseCost(slot.durationMinutes, slot.lessonKind, hourlyRate);
    if (baseCost === null) {
      unpricedSlots += 1;
      continue;
    }
