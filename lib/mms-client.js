...
class MMSClient {
  constructor() {
    ...
    this.token = process.env.MMS_BEARER_TOKEN || process.env.MMS_DEFAULT_TOKEN || '';
  ...
  setToken(token) {
    this.token = token || process.env.MMS_BEARER_TOKEN || process.env.MMS_DEFAULT_TOKEN || '';
  }

  async fetchFromMMS(endpoint, method = 'GET', body = null, options = {}) {
    const token = this.token || process.env.MMS_BEARER_TOKEN || process.env.MMS_DEFAULT_TOKEN || '';
