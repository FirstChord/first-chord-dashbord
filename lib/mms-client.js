...
import { buildTutorTeacherIdMap, resolveTutorTeacherId } from './tutor-dashboard-helpers.mjs';

class MMSClient {
  constructor() {
    this.baseUrl = 'https://api.mymusicstaff.com/v1';
    this.schoolId = 'sch_Fx5JQ';
    this.token = getMmsBearerToken();
    
    // Derived from ADMIN_TUTORS so the tutor dashboard, admin workflows, and MMS
    // roster filter use the same teacher IDs.
    this.teacherIds = buildTutorTeacherIdMap();
  }

  setToken(token) {
    this.token = token || getMmsBearerToken();
  }

  async fetchFromMMS(endpoint, method = 'GET', body = null, options = {}) {
    const token = this.token || getMmsBearerToken();
  ...
  async getStudentsForTeacher(tutorName = null) {
    const teacherId = resolveTutorTeacherId(tutorName);
    ...
    if (result.success && result.data && result.data.ItemSubset) {
      ...
      const teacherStudents = result.data.ItemSubset.filter(student => {
        // Student must be active (MMS exposes this under a few property names).
        ...
        // A student belongs to this teacher if EITHER their StudentGroups OR their
        // BillingProfiles link to the teacher. Check both and keep them if either
        // matches — do NOT early-return on StudentGroups. Some MMS records carry an
        // empty/teacherless StudentGroups entry (e.g. `[{}]`) while the real teacher
        // assignment lives in BillingProfiles; an early return on StudentGroups would
        // skip the BillingProfiles check and silently drop an active student from their
        // tutor's dashboard. See docs/history/logs/learning-log.md (Santi Freeth).
        const inStudentGroups = (student.StudentGroups || []).some(group => {
          const notEnded = !group.EndDate || new Date(group.EndDate) > new Date();
          return group.TeacherID === teacherId && group.IsActive && notEnded;
        });
        const inBillingProfiles = (student.BillingProfiles || []).some(profile => (
          profile.TeacherID === teacherId && profile.Active !== false
        ));

        return inStudentGroups || inBillingProfiles;
      }).map(student => {
