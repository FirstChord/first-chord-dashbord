// GENERATED — do not edit directly. Run: npm run generate-configs to regenerate.
/**
 * Theta Music Trainer Credentials
 *
 * Maps MMS student IDs to their Theta Music login credentials.
 * Username and password are the same for all students.
 */

export const thetaCredentials = {
  // Arion's students
  'sdt_fyYPJK': 'oliviamcintoshguitarfc', // Olivia Mcintosh
  'sdt_yXFSJf': 'indyfc', // Indy Norman
  'sdt_yXFnJm': 'roarfc', // Roar Norman
  'sdt_m1P8Js': 'cobanfc', // Coban Kousourou
  'sdt_m941JY': 'peytonfc', // Peyton Okorodudu
  'sdt_mZHMJH': 'sajafc', // Saja Elfaki
  'sdt_L8Y3JY': 'sadielinfc', // Sadie Lindop
  'sdt_mGY6JY': 'enolfc', // Enol Pardo-Alguero
  'sdt_mvK6JZ': 'juliedfc', // Julie Donaldson
  'sdt_f2zpJ3': 'kianojaghifc', // Kian Ojaghi
  'sdt_fytnJT': 'arvinojaghifc', // Arvin Ojaghi

  // Calum's students
  'sdt_m7yfJG': 'samfc', // Sam Reid
  'sdt_fgBHJl': 'torosfc', // Toros Erskine
  'sdt_fcs0JQ': 'aaratrikaghatkarfc', // Aaratrika Ghatkar

  // Chloe's students
  'sdt_6Zd7J3': 'niamhfc', // Niamh McCrudden
  'sdt_6nhSJM': 'giorgiofc', // Giorgio O
  'sdt_H5yHJQ': 'charliefc', // Charlie Gillespie
  'sdt_HVbNJk': 'melaniafc', // Melania R
  'sdt_MdD4JB': 'falloufc', // Fallou Ndiaye
  'sdt_fFZdJb': 'tabithafc', // Tabitha Slocombe
  'sdt_c8fFJ3': 'emmafc', // Emma Snaraite
  'sdt_y0SPJJ': 'zoefc', // Zoe Bannatyne
  'sdt_cRXfJT': 'thomasfc', // Thomas McGrath
  'sdt_LXBCJ0': 'rianfc', // Rian Wilson
  'sdt_mqLVJ3': 'mavifc', // Mavi Ayhan
  'sdt_fw0cJ3': 'sandysingingmorganfc', // Sandy (Singing) Morgan
  'sdt_fwnBJg': 'miamackinnonpianofc', // Mia Mackinnon (piano)
  'sdt_fK3nJ9': 'vivianschmiedeckefc', // Vivian Schmiedecke
  'sdt_fHpTJ6': 'emelinequillienvisserfc', // Emeline Quillien-Visser
  'sdt_frnpJH': 'aylamehmoodfc', // Ayla Mehmood
  'sdt_fkZbJV': 'andrzejishizakifc', // Andrzej Ishizaki
  'sdt_fb4YJV': 'louisebolandfc', // Louise Boland

  // David's students
  'sdt_L5l5JD': 'dimitrafc', // Dimitra Ntarmou
  'sdt_LqT2J4': 'fraserfc', // Fraser Ord
  'sdt_MhHLJ0': 'rebeccafc', // Rebecca Mapata
  'sdt_pGqXJ9': 'silverrayfc', // Silver-Ray Noramly
  'sdt_m4rFJC': 'sineadfc', // Sinead Smollett
  'sdt_fl9nJD': 'theodorehendersonfc', // Theodore Henderson

  // Dean's students
  'sdt_2lJMJl': 'mohammedfc', // Mohammed Abdelrahman
  'sdt_2lJyJj': 'rayanfc', // Rayan Abdelrahman
  'sdt_2sjZJs': 'harrisonfc', // Harrison Aitken
  'sdt_3TTwJV': 'emiliafc', // Emilia Douglas
  'sdt_3nKZJB': 'dylanfc', // Dylan Cook
  'sdt_BDHTJN': 'danielafc', // Daniela Alvarez
  'sdt_Fq8ZJ1': 'yahyafc', // Yahya Faraj
  'sdt_Hf36Jh': 'arianafc', // Ariana Honeybunn
  'sdt_HrdXJK': 'gilfc', // Gil Wallace
  'sdt_NxMZJz': 'zaynfc', // Zayn Speirs
  'sdt_QnkRJT': 'adafc', // Ada Neocleous
  'sdt_BDHqJv': 'alicefc', // Alice Drew
  'sdt_cYDxJM': 'jamesfc', // James Taylor
  'sdt_cfbyJQ': 'harryfc', // Harry Wallace
  'sdt_csfBJd': 'charlottefc', // Charlotte Lawrie
  'sdt_sfbtJ5': 'stellafc', // Stella Hart
  'sdt_svMqJy': 'lolafc', // Lola McGarry Panunzio
  'sdt_vHS3JK': 'sandyfc', // Sandy Morgan
  'sdt_yHtZJ8': 'danielfc', // Daniel Murray
  'sdt_cCw9Jz': 'matteomalarafc', // Matteo Malara

  // Eléna's students
  'sdt_2grxJL': 'ryanfc', // Ryan Ofee
  'sdt_6RJkJp': 'omarfc', // Omar Mukhtar
  'sdt_D9rnJT': 'rohanfc', // Rohan Nazir
  'sdt_HlXyJl': 'sophiafc', // Sophia Papadakis
  'sdt_Kq3RJW': 'pablofc', // Pablo Cunningham
  'sdt_M3RnJG': 'athenafc', // Athena Papadakis
  'sdt_NSmFJh': 'alessandrofc', // Alessandro Matassoni
  'sdt_NSmyJ3': 'ziafc', // Zia Permall
  'sdt_NXNKJJ': 'evafc', // Eva Lindsay
  'sdt_vcJPJj': 'katerinafc', // Katerina Skouras
  'sdt_yFzkJ6': 'irrujfc', // Irruj Chander
  'sdt_yhJTJ0': 'vaidikfc', // Vaidik Gupta
  'sdt_LpvnJ9': 'astridfc', // Astrid Jonsson

  // Fennella's students
  'sdt_39hsJY': 'chiarafc', // Chiara Cavanna
  'sdt_BDH9J3': 'ceitdhfc', // Ceitdh Qui
  'sdt_HWVBJL': 'alizefc', // Alize Ekdi
  'sdt_Mg2kJz': 'annafc', // Anna Kennedy
  'sdt_QpGPJv': 'alexanderfc', // Alexander Murray
  'sdt_cB0YJd': 'tormadfc', // Tormad MacRobert
  'sdt_cGPBJ3': 'oliviafc', // Olivia Mcintosh
  'sdt_ccgQJ8': 'paisleyfc', // Paisley Hewitt
  'sdt_cfGhJh': 'sianfc', // Sian Malyin
  'sdt_v1lcJ0': 'maxfc', // Max Toner
  'sdt_v9m8JT': 'elizafc', // Eliza Dem
  'sdt_ySKCJy': 'dylanfc', // Dylan Lyall
  'sdt_ybcdJ7': 'elizabethfc', // Elizabeth MacKenzie
  'sdt_yLBVJQ': 'elliotfc', // Elliot
  'sdt_yLBsJ7': 'leofc', // Léo TERHZAZ
  'sdt_LfgNJC': 'solomonfc', // Solomon Nazir
  'sdt_L1NQJl': 'niamhwfc', // Niamh Walker
  'sdt_Ll4DJt': 'aislingfc', // Aisling Mackenzie
  'sdt_mvhgJ3': 'patriciafc', // Patricia Miranda
  'sdt_f2VgJD': 'mariamfarhanifc', // mariam farhani
  'sdt_ff2SJf': 'annamcphailfc', // Anna McPhail

  // Finn's students
  'sdt_2slYJ7': 'jofc', // Jo Vardy
  'sdt_BDHFJM': 'arnavfc', // Arnav Rekhate
  'sdt_BDHJJF': 'clairefc', // Claire McGinniss
  'sdt_BDHKJs': 'archiefc', // Archie Toner
  'sdt_BDHdJ4': 'thomasfc', // Thomas Ward
  'sdt_BDsMJk': 'teaganfc', // Teagan Samuel
  'sdt_BDsRJ9': 'rosefc', // Rose Drew
  'sdt_K3h1JJ': 'ariafc', // Aria Thomson
  'sdt_K9pNJt': 'joelfc', // Joel Adler
  'sdt_K9psJ9': 'hudsonfc', // Hudson Woodford
  'sdt_KKfGJ0': 'leonardofc', // Leonardo Matassoni
  'sdt_KKfZJC': 'simonefc', // Simone De Maio
  'sdt_Kq3XJP': 'laithfc', // Laith Lombard
  'sdt_Kv59Jb': 'eilidhfc', // Eilidh Su Strachan
  'sdt_Kv5QJh': 'katrinafc', // Katrina Caldwell
  'sdt_Kv5XJL': 'santifc', // Santi Freeth
  'sdt_Kwb2J0': 'anjifc', // Anji Goddard
  'sdt_KwbMJR': 'carolynfc', // Carolyn Hilliard
  'sdt_KwbvJG': 'alisterfc', // Alister McGhee
  'sdt_N0zrJ8': 'josefc', // Jose Santi Dad
  'sdt_cZDlJQ': 'Finnlayfc', // Finlay Cackett
  'sdt_gWXHJN': 'nathanfc', // Nathan Ward
  'sdt_gwyQJr': 'alexfirstchord', // Alex Chang
  'sdt_x48LJT': 'stellafc', // Stella Cook
  'sdt_yLv3J7': 'sakethfc', // Saketh Pamidimarry
  'sdt_L15vJh': 'guyfc', // Guy Pilsworth
  'sdt_fBg9JN': 'teststudentyfc', // Test Studenty
  'sdt_v2XJJZ': 'paulmaherfc', // Paul Maher
  'sdt_flBhJR': 'clairemcginnessukulelefc', // Claire  McGinness (Ukulele)

  // Ines's students
  'sdt_N0z0Jq': 'tomassofc', // Tomasso Fossati
  'sdt_c794J5': 'mateofc', // Mateo Alonso
  'sdt_LP9GJp': 'markmfc', // Mark Mitchell
  'sdt_mqL5JN': 'liamfc', // Liam Hopton
  'sdt_m3mmJB': 'fadimefc', // Fadime Kara
  'sdt_my8xJB': 'innesfc', // Innes Morton

  // Kenny's students
  'sdt_L9nZJs': 'craigfc', // Craig Mcaulay
  'sdt_cZsMJD': 'ninabfc', // Nina Brown
  'sdt_mYNRJx': 'lloydfc', // Lloyd Raymond
  'sdt_LP3gJ5': 'astonfc', // Aston Black
  'sdt_mNd0J8': 'gerryfc', // Gerry Morrison
  'sdt_LXQhJB': 'nevisfc', // Nevis Porecki
  'sdt_mMtGJ5': 'ruqafc', // Ruqa Almotabaqi
  'sdt_f50CJ3': 'emilyfc', // Emily Grifa
  'sdt_f576Jx': 'ninagavlinfc', // Nina Gavlin
  'sdt_fn5wJR': 'loganpetriefc', // Logan Petrie

  // Kim's students
  'sdt_38MBJF': 'markfc', // Mark Chalmers
  'sdt_BkflJy': 'reubenfc', // Reuben Barnes
  'sdt_HbXMJZ': 'lyrafc', // Lyra Jackson
  'sdt_QbBNJq': 'carolinefc', // Caroline Bingley
  'sdt_QcCtJT': 'marcofc', // Marco Sarconi
  'sdt_mfFYJ0': 'nicholaswylliefc', // Nicholas Wyllie

  // Patrick's students
  'sdt_QP01Jp': 'yarahfc', // Yarah Love
  'sdt_QSzJJ2': 'arasfc', // Aras Korkmaz
  'sdt_QfWBJx': 'eddiefc', // Eddie Roarty
  'sdt_cJDjJj': 'suzannefc', // Suzanne Boyle
  'sdt_cYvdJp': 'noahfc', // Noah Hegde
  'sdt_mBr8J6': 'williamfc', // William McCormick
  'sdt_L6LLJ3': 'ezrafc', // Ezra Ford
  'sdt_L9tMJD': 'rumbiefc', // Rumbie M
  'sdt_mv22J8': 'ailsamfc', // Ailsa Macfarlane
  'sdt_LfgQJd': 'lynseyfc', // Lynsey Lamond
  'sdt_mSDKJ1': 'lachlanfc', // Lachlan Venturi
  'sdt_fGR3Jk': 'jasleenraifc', // Jasleen Rai

  // Robbie's students
  'sdt_2s8CJk': 'charliefc', // Charlie Mcdougall
  'sdt_3ZZwJ2': 'ailsafc', // Ailsa Hoebe
  'sdt_c7hKJ8': 'emilianofc', // Emiliano Squillino
  'sdt_vgjdJv': 'niamhfc', // Niamh Gallagher
  'sdt_vrbPJL': 'foxfc', // Fox Slocombe
  'sdt_w6T7Jd': 'vailafc', // Vaila Donaghey
  'sdt_w6TSJX': 'arjunfc', // Arjun Darshini

  // Scott's students
  'sdt_635GJ0': 'ritishafc', // Ritisha Paryani
  'sdt_LpXfJQ': 'ninahfc', // Nina Hamilton
  'sdt_LNvQJ7': 'albafc', // Alba McMillan
  'sdt_cZsDJp': 'katiefc', // Katie Brown
  'sdt_fsB1Jm': 'gaiaknightfc', // Gaia Knight
  'sdt_mZbRJh': 'marthajeffersonfc', // Martha Jefferson
  'sdt_fcSVJh': 'emelynfullerfc', // Emelyn Fuller

  // Stef's students
  'sdt_NS6bJW': 'tabithafc', // Tabitha Slocombe
  'sdt_NSmPJr': 'avafc', // Ava Garcia Bree
  'sdt_NdzdJM': 'elsafc', // Elsa Thorton
  'sdt_czFfJy': 'bonniefc', // Bonnie Clark
  'sdt_s2JpJx': 'roquefc', // Roque Neto
  'sdt_wwqLJ2': 'jackfc', // Jack Alexander
  'sdt_fkkLJ0': 'lawrencegalliofc', // LAWRENCE GALLIO

  // Tom's students
  'sdt_417JJ3': 'charliefc', // Charlie Norton
  'sdt_6PjKJF': 'ruaraidhfc', // Ruaraidh Somerville
  'sdt_BMtHJs': 'lukefc', // Luke Moran
  'sdt_BtxmJ4': 'carolfc', // Carol Turner
  'sdt_D9ftJB': 'sadiefc', // Sadie Lowe
  'sdt_DdxtJZ': 'rosiefc', // Rosie Ward
  'sdt_F3FHJs': 'loganfc', // Logan Wilson
  'sdt_FXKDJS': 'cormacfc', // Cormac Morton
  'sdt_Nt4LJ3': 'stellafc', // Stella French
  'sdt_QjMGJc': 'carlafc', // Carla Jurado
  'sdt_cZ39Jz': 'anniefc', // Annie Brown
  'sdt_gDcVJp': 'lucafc', // Luca Kennedy
  'sdt_gpFVJS': 'dukefc', // Duke Noramly
  'sdt_pFYFJT': 'rosiekfc', // Rosie Kinniburgh
  'sdt_pT5MJz': 'sonnyfc', // Sonny Ford
  'sdt_slc4Jq': 'harryfc', // Harry Dafas
  'sdt_vGjtJ7': 'catfc', // Cat Macfarlane
  'sdt_w6TBJ3': 'rowanfc', // Rowan Moore
  'sdt_417tJM': 'ashfc', // Ash Dallas Gray
  'sdt_DP20Js': 'evanfc', // Evan Gibson

};
