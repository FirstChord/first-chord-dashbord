...
export const cache = {
  ...
  getStudents: (tutor) => {
    try {
      ...
      if (
        Array.isArray(data)
        && data.some((student) => student?.mms_id || student?.ID)
        && !data.some((student) => student?.noteAccessToken || student?.note_access_token)
      ) {
        console.log(`🔐 Cache missing note access tokens for tutor: ${tutor}`);
