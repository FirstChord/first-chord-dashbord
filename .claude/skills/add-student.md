...
2. **Generate the FC Student ID first**
   Use the MMS student ID as the seed:
   ```bash
   python3 -c "import hashlib; mms='sdt_XXXXXX'; print('fc_std_' + hashlib.sha256(mms.encode()).hexdigest()[:8])"
   ```
   Replace `sdt_XXXXXX` with the actual MMS ID. Keep the result for step 3.

3. **Update Registry JavaScript File**
   ...
   'sdt_XXXXXX': {
     ...
     fcStudentId: 'fc_std_XXXXXXXX',                            // from step 2
   }, // Alice Drew
   ```
